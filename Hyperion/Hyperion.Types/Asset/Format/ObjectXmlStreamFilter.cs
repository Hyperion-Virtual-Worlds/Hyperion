            ...
            while (count > 0)
            {
                ...
                if (m_Buffer[m_BufUsed] == (byte)'<')
                {
                    ...
                    if (test.StartsWith("<SceneObjectPart"))
                    {
                        ...
                        {
                            /* stupid mono XmlTextReader and XmlTextWriter
                             * when using as specified it adds more and more
                             * xmlns after each iteration when used as filter
                             * parser and writer. 
                             * 
                             * And this was an OpenSim misuse since it mistook
                             * LocalName and Name. Pushing Name into LocalName
                             * is just a misuse of the API.
                             */
                            int pos;
                    ...
                    {
                        /* 
                         * OpenSim's XML declarations are actually
                         * incorrect, so in order to be sure that
                         * we get things correct and in-line with
                         * Secondlife and AGB we need to just ignore
                         * the OpenSim XML declarations.
                         * 
                         * Addtionally we need to filter every other
                         * tag, because OpenSimulator only uses UTF-8
                         * but falsely declares some tags as UTF-16.
                         */
